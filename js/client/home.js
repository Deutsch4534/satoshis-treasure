...
Home.startGame = function(){
    ...
    request.done(function (data) {
        ...
        $('#payreq').attr('src','https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=' + data.request);
        invoiceId = data.id
        toast('warning', 'Starting the game costs 1000 satoshis')
        setTimeout(function(){
...
$(document).ready(function () {
    ...
    $('#check-button').on('click', function () {
        ...
        request.done(function (data) {
            if (data.paid) {
            $('#modal').modal('hide');
            toast('success', "Invoice Paid. Let's start!!")
            var ok = true;
            if(Game.isNewPlayer) {
                if(!Home.isNameEmpty()){
